...
Note: we had to create a separate folder for pipelines because we didn't want to use Argo, which comes with a Piplines module. If you can use default Argo, then you can simply use `https://github.com/kubeflow/manifests/apps/pipeline/upstream/env/platform-agnostic-multi-user-pns?ref=v1.3.0` instead of the pipelines folder.
...
Note: Intuit SSO supports OIDC, so we did not need to use dex for the integration. If your org's SSO does not support OIDC, then you can use dex in the middle; details can be found [here](https://www.kubeflow.org/docs/started/k8s/kfctl-istio-dex/).
...
2. Seting up ACM certificate for the host name you want to use for the Kubeflow installation, host name can be `kubeflow.example.com`
...
1. Check if EnvoyFilter is getting applied: you should have the **istioctl** cmd tool, which makes the job much to install using brew:
