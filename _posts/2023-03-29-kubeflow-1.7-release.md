...
1.7 includes hundreds of new commits, and the following provides details of the top features.  You will also find updates on platform dependencies, changelogs, breaking changes, ecosystem partners (Ray, MLflow and BentoML), the new Kubeflow community security team and Kubeflow transition to the CNCF. We encourage active users to contribute to our next release, and provide links to ways to join the community as well.
...
Katib includes new enhancements to the UI and SDK. The new Katib UI provides simplified fine-grained configuration and log correlation. Additionally sorting and filtering have been added, allowing a more organized view of your many experiments. These features also reduce the need to manually use low level commands to find and correlate logs with hyper parameter experiment configurations, simplifying in depth performance analysis and subsequent model parameter iterations.  
...
Additionally, 1.7 provides and update to the Notebooks form page [#6826](https://github.com/kubeflow/kubeflow/pull/6826) as well as dedicated page [#6769](https://github.com/kubeflow/kubeflow/pull/6769) [#6788](https://github.com/kubeflow/kubeflow/pull/6788) for the different types of tools managed by Kubeflow. These details pages allow users to view logs, events and configuration yamls and they also link from one another (i.e. going to a volume’s details page via a notebook details page). Previously these functions were only available through the Kubernetes API,  which would require the user increased privileges and more in-depth knowledge of Kubernetes CLI commands. With these new features the user has a simpler, more organized and more secure way of accessing crucial information around their Kubeflow resources.

Other notable features are small improvements on our user stories around PodDefaults. Aside from additional use-cases, like defining sidecar and init containers [#6749](https://github.com/kubeflow/kubeflow/pull/6749), Kubeflow’s TensorBoard stack now integrates with PodDefaults [#6874](https://github.com/kubeflow/kubeflow/pull/6874) [#6924](https://github.com/kubeflow/kubeflow/pull/6924). This means that users can re-use their existing PodDefaults, i.e. for gaining access to S3, for both Notebook and TensorBoard servers.
...
2. Created an [inventory](https://github.com/kubeflow/manifests/pull/2419) of container images used by Kubeflow
