...
### Security - Rootless Kubeflow Updates and Istio CNI

In our efforts to enhance security within the project, version 1.8 continues to work in rootless kubeflow. Proposed and developed by Julius von Kohout [(#2455)](https://github.com/kubeflow/manifests/pull/2455), v.1.8 offers the flexibility to run kubeflow either with or without the optional istio-cni. Additionally, it enforces [Pod Security Standards](https://kubernetes.io/docs/concepts/security/pod-security-standards/) base/restricted to manifests/contrib that ensure default security measures when operating within a Kubernetes cluster. Notably, using istio-cni eliminates the need for a sidecar with root-level privileges. Future versions will continue to refine this functionality, with plans to make istio-cni the default option. This will further reduce kubeflow's vulnerability to CVEs.
