...
**Note** 
Another way to submit an experiment is by using the [Katib SDK][Katib_SDK].

Katib provides a [`tune`][tune_api] API that allows you to pass the experiment specifications as parameters. The objective function 
can internally call the optimization script from STEP 2. However, make ensure that the script is included in the container 
image specified in the tune API. 
