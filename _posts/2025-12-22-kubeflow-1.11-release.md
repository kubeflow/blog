---
title: "Kubeflow 1.11 Release Announcement"
layout: post
toc: false
comments: true
...
A new Model Catalog feature enables model discovery and sharing with governance controls. Teams can browse and register models from external sources such as Hugging Face, with an admin settings page for managing catalog sources, filtering, and model visibility. The catalog UI and backend are under active development.

### KServe Integration

- **Custom Storage Initializer (CSI)**  Enables model download and deployment using model metadata directly from the Registry.
- **Reconciliation loop**  Observes KServe InferenceServices to automatically populate Model Registry logical-model records, keeping registry metadata in sync with live deployments.
...
- **Async Upload Job**  A Kubernetes Job for transferring and packaging models (including OCI format), simplifying model storage operations in production environments.

### Additional Improvements

- Removal of the legacy Google MLMD dependency.
- Support for Experiment and Run tracking entities.
- PostgreSQL support alongside MySQL.
- Multi-architecture container builds (amd64/arm64).
- SBOM generation and OpenSSF Scorecard CI integration.

## Training Operator (Trainer) & Katib

Kubeflow 1.11 includes Training Operator v2.1.0, a major architectural evolution that simplifies distributed training on Kubernetes with a unified API, Python-first workflows, and enhanced LLM fine-tuning capabilities.

### New API Architecture

Training Operator v2 introduces **TrainJob**  a unified training job API that replaces framework-specific CRDs (PyTorchJob, TFJob, etc.). Infrastructure configuration is now separated into **TrainingRuntime** and **ClusterTrainingRuntime** resources, creating a clean boundary between platform engineering (runtime setup) and data science (job submission).
...
- **Helm Charts**  Deploy with `helm install kubeflow-trainer oci://ghcr.io/kubeflow/charts/kubeflow-trainer`.
...
- **Kueue**  Topology-aware scheduling for TrainJobs, enabling optimal placement for distributed training across node groups.
...
Katib hyperparameter tuning remains compatible with Training Operator v2, allowing users to optimize model hyperparameters alongside the new training workflow.
...
Visit the Kubeflow 1.11 [release page](https://github.com/kubeflow/manifests/releases) or head over to the Getting Started and Support pages.
