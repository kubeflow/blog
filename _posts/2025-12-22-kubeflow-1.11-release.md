...
Optional in-memory cache cluster (powered by [Apache Arrow](https://arrow.apache.org/) and [Apache DataFusion](https://datafusion.apache.org/) ) streams datasets directly to GPU nodes with zero-copy transfers, maximizing GPU utilization and minimizing I/O wait times for large-scale training workloads. More details can be found [here](https://www.kubeflow.org/docs/components/trainer/user-guides/data-cache/).
